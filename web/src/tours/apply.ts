...
/**
 * The store patch that lands the app in a tour step's state.
 *
 * A step can change n, l, m, system, config, model, exchange and pauli in one
 * move, so this has to clear the union of what every individual action clears,
 * not just INVALIDATED. Each action spreads `INVALIDATED` itself rather than
 * having it applied in `setState`, so a raw `setState(stepState(step))` would
 * leave the previous step's cloud, plane, surface, levels and spectrum in the
 * store and render them under this step's labels. `classicalGhost` (cleared by
 ...
 * are named here.
 *
 * INVALIDATED goes after the step's own state, which matters for the one field
 * they share: `profileZoom`. A zoom window names a line in a spectrum this
 * step has just discarded, so clearing it is the right way round.
 *
 * The model resolves exactly as setSystem resolves it: sulfur and chlorine
 ...
 *
 * `compare` deliberately does not resolve here. resolveCompare reads false
