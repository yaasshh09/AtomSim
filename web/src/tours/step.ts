...
/**
 * The full state a step asks for.
 *
 * A full reset from the defaults, never a patch onto whatever the previous
 * step left behind. Two reasons: a step's picture is then reproducible from
 * that step's own data, which is what makes its claims mean anything, and
...
/** An index safe inside the tour, whatever a hand-edited `?step=` supplied. */
