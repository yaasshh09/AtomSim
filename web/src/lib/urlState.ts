/* Deep links are the demo-script hook surface (spec M4): every state the Phase
 * 2 guided tour needs is addressable by URL alone. Parsing validates hard, and
 * a junk parameter gets dropped rather than reaching the store. */
...
export interface UrlState {
  ...
  dirac: boolean;
  /** Whether both models' total density share one axis in the Radial view. Off by default. */
  compare: boolean;
  bField: number;
  eField: number;
  hyperfine: boolean;
  /** Whether the Spectrum view draws line strengths. On by default, so the URL marks it off. */
  intensities: boolean;
  /** Whether the Spectrum view applies LTE weighting. Off by default, so the URL marks it on. */
  thermal: boolean;
  temperatureK: number;
  /** log10 of the electron density in cm^-3. Stored as the log because the
   *  control is a log slider, and round-tripping 1e13 as a decimal string
   *  would lose precision for nothing. */
  logNe: number;
  /** Whether the Spectrum view synthesizes Voigt line profiles. Off by default. */
  profile: boolean;
  /** log10 of the spectrograph resolving power, or null when there is no
   *  instrument. Stored as the log for the same reason as logNe: the control
   *  is a log slider. */
  logResolvingPower: number | null;
  /** The wavelength window [nm] a profile is zoomed to, or null for the full
   *  range. Carried so a link can point at one line's shape, which is the
   *  whole reason the zoom exists. */
  profileZoom: [number, number] | null;
  /** Whether the Spectrum view draws the absorption spectrum. Off by default. */
  absorption: boolean;
  /** log10 of the column density in m^-2. Carried because it is a physical
   *  knob and not a disclosure toggle: moving it walks the gas from a faithful
   *  census to a saturated one, and a link to "hydrogen at 10^22, Lyman black
   *  and Balmer invisible" has to survive being shared. */
  logColumn: number;
  ghost: boolean;
  ...
  forceL: number;
  /** The custom force-law expression V(r) to solve; only meaningful when forcePreset==="custom" */
  forceExpr: string;
  /** The screened-atom electron configuration; null means the Aufbau ground state (the default) */
  config: string | null;
  /**
   ...
   * the store invalidates on it.
   *
   * The default is "gsz", so every deep link written before Hartree-Fock
   * existed keeps resolving to the physics it was written against.
   *
   * The key is `model`. Note that `hf` is used separately as the *key* for the
   ...
   * returned as COUNTERFACTUAL. It serializes as `nox=1` rather than
   * `exchange=0`, and that polarity is deliberate, so ABSENCE means real
   * physics: a link that predates this toggle, or one trimmed by hand, cannot
   * land anyone in altered physics by omission.
   *
   * `nox` and not `x`: `hf` is already taken by hyperfine, and this file has
   * one near-collision too many already.
   */
  exchange: boolean;
  /**
   * Whether the Hartree-Fock solve keeps the occupancy cap.
   *
   * False collapses the configuration to 1s^N. It serializes as `nopauli=1`,
   * same polarity and same reason as `nox`: absence means real physics, so no
   * link written before this existed can drop anyone inside a counterfactual.
   *
   ...
   * a state worth honouring literally, since it names a model the API rejects,
   * so it reads as the collapse it obviously means.
   */
  pauli: boolean;
  /**
   * The tour in progress, and how far into it the reader is.
   *
...
export const URL_DEFAULTS: UrlState = {
  ...
  absorption: false,
  logColumn: 20,
  // Opens in a stellar photosphere: warm enough that the excited levels are
...
// the short URL names for the five constant multipliers (m_e -> "me")
 ...
 *
 * It lives here rather than in main.tsx because two callers now need the same
 ...
 * second hand-maintained copy of thirty-seven field names is how those two
 * drift.
 */
export function currentUrlState(s: Omit<UrlState, "tour" | "step">): UrlState {
  return {
    ...
    exchange: s.exchange,
    pauli: s.pauli,
    // Not read off the snapshot. The store holds them as tourId/stepIndex, and
    // the two callers want opposite things: the URL subscriber overrides both
    // with the live tour, while the tour's own entry snapshot wants the
    // reader's state without a tour in it, since that is what gets restored on
    // exit.
    tour: null,
    step: 0,
  };
...
/** The validated partial state read out of a query string. Invalid params are dropped. */
export function parseAppUrl(search: string): Partial<UrlState> {
  ...
  // Dropped rather than thrown, like every other parameter here: the contract
  // is that junk never reaches the store, and a link with a typo should still
  // open the app rather than fail to render.
  const model = pickEnum(q.get("model"), MODELS);
  if (model) out.model = model;

  if (q.get("nox") === "1") out.exchange = false;
  // The cap and antisymmetry are one rule, so `nox` rides along with
  // `nopauli=1` whether or not the link says so. Honouring a bare
  // `?nopauli=1` literally would build a request the API answers 422 to, which
  // is a worse reading of the intent than the obvious one.
  if (q.get("nopauli") === "1") {
    out.pauli = false;
    out.exchange = false;
  }
  ...
  // Mirrors the store guard: phase needs the complex basis
  ...
  // The interval is held open, hard: 0 and 1 are not contours, and the server
  // would answer 422 for either. A junk value drops back to the default rather
  // than travelling into a request.
  ...
  // The param is "ef" (electric field): "e" is already the charge multiplier (CONST_PARAMS)
  const ef = Number(q.get("ef"));
  if (Number.isFinite(ef) && ef > 0) out.eField = ef;

  if (q.get("hf") === "1") out.hyperfine = true;

  // On by default, so only the off state travels: "int=0".
  ...
  // The same bounds as the slider. Outside them the engine still answers, but
  // the spectrum is either a flat line or entirely black, with nothing to
  // show.
  ...
  // The force-law preset and its params are one axis, independent of the main
  // physics. The preset selects which param names are live; each one is
  // validated and clamped to its own spec range, with the preset defaults
  // filling in anything missing. These fields are written only when the URL
  ...
  // The custom force-law expression is read only for the custom preset, and
  // only when the lightweight browser check passes (the server's AST parser is
  // the authority).
  ...
  if (tour) {
    ...
    // Clamped to the tour's real length in clampStep at apply time. Here it
    // only has to be a non-negative integer, since this module cannot know how
    // long any tour is.
...
/** The canonical query string for a state. Default values are omitted ("" if everything is default). */
export function serializeAppUrl(state: UrlState): string {
  ...
  // Written only when a surface is being drawn: a fraction in a link that
  ...
  // Written whenever it is non-default, including for hydrogenic systems where
  // it currently selects nothing. Dropping it there would make the round trip
  // depend on which system is loaded, and a link is supposed to carry the
  // state it was written from.
  ...
  // through a parser that would infer the missing one anyway, but a link
  // someone reads or edits should say both things it means.
  if (!state.pauli) q.set("nopauli", "1");
  if (state.tour) {
    q.set("tour", state.tour);
    if (state.step !== URL_DEFAULTS.step) q.set("step", String(state.step));
  }
  // note: '+' stays percent-encoded (%2B), because a literal '+' in a query
