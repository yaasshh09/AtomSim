 ...
 *
 * A statement about the physics rather than about the rendering, so it is not
 * a disclosed liberty and does not go through lib/liberties.ts. The
...
/** The part of the state a job payload needs to name its many-electron model. */
...
/**
 * The four fields sent with every picture job.
 *
 * Under the screened model the counterfactual flags are forced back to real
 * physics. GSZ has no exchange term to remove and no occupancy cap of its own,
 ...
 * atom nobody has reported yet, and greying a control on a guess is worse than
 * greying it a moment late. Also true for every hydrogenic preset, which no
 * screened model touches.
 */
...
/**
 * The model this atom actually gets solved with.
 *
 * Sulfur and chlorine have no GSZ parameters, so leaving "gsz" selected there
 * would refuse every request with a 400 the picker could have avoided. This
 * applies both when the system changes and when the system table arrives,
 ...
 *
 * It takes a many-electron atom (a one-electron system has no total density
 * for either model) and GSZ parameters (sulfur and chlorine have none). False
 * while the system table is still loading, which is the opposite default from
...
/** Forces a deep-linked `compare=1` off where it cannot run. */
 ...
 * firing a job already known to come back 422.
 *
