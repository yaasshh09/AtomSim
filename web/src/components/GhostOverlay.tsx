...
const GHOST_COLOR = "#8be9fd"; // classical cyan, kept distinct from the |psi|^2 LUTs
...
/**
 * The collapse spiral, sampled uniformly in swept angle rather than in tau:
 * the winding rate diverges as tau -> 1, so uniform-tau sampling would starve
 * the inner turns. With u = 1 - sqrt(1 - tau), theta is linear in u.
 */
...
/**
 * The three.js overlay rendered inside the CloudView <Canvas>: Bohr rings, the
 * collapse spiral, and one animated ghost point. The radii are raw bohr,
 * exactly the units the point cloud is drawn in. `distance` (the camera
 * distance) only sizes the ghost point, mirroring the nucleus-marker pattern.
 *
 * The animated point is driven through a three.js ref in useFrame, so React
 * never re-renders per frame. `tauRef` shares the live loop phase with the HUD
 * clock outside the Canvas, which polls it at ~10 Hz.
 */
export function GhostOverlay({
  ...
  // VISUAL LIBERTY, disclosed through CLASSICAL_SLOWMO: the drawn winding
  // count is capped, because the honest count (~1e5) would alias into noise at
  // any frame rate.
...
/**
 * The live simulated-time clock for the HUD, outside the Canvas. It polls the
 * shared tau ref at 10 Hz on its own interval: a tiny local re-render, never
