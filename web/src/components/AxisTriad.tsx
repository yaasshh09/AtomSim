 ...
 * without an axis to read it against, the point cloud cannot say which one it
 * is. `RENDER_LIBERTIES` has always declared that z is drawn screen-vertical;
 * this draws the axis that claim is about.
 *
 * Both halves of each axis get drawn. A single arrow would suggest a preferred
...
/** The arm length, as a fraction of the camera framing distance. */
const ARM = 0.42;
...
export function AxisTriad({ distance }: { distance: number }) {
  ...
  return (
    // Inside the same rotation as the cloud and the isosurface, so the vectors
    // below are engine axes rather than three.js ones. Without it, the
    // screen-vertical axis was labelled "y" here while the cloud beside it and
    ...
    <group rotation={PHYSICS_TO_SCREEN}>
      {AXES.map((a) => (
        <Line
          ...
          transparent
          opacity={0.5}
          // The cloud blends additively and the axes do not. Without this, an
          // axis behind a dense lobe would draw over the top of it and read as
          // a line in front.
          depthWrite={false}
        />
      ))}
