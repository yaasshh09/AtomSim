...
/* Tuned to the instrument palette: EXACT gets the shell's own mint, so the
   ...
   stays exactly where it is, because index.css uses that same hue for the
   ghost HUD, the counterfactual banner and the counterfactual rungs, and a
   badge that drifted off it would stop matching the thing it labels. */
...
const MARGIN = 8;
/** The gap between a badge and the panel it opens, in px. */
const GAP = 6;

type Point = { left: number; top: number };

/* The panel goes below the badge when it fits there, above it when it does
   not, and pinned to the window when neither side has room: on a short
   viewport that shows the top of the panel and scrolls the rest, rather than
   running half of it off-screen. Horizontally the panel starts at the badge
   and slides back in whenever that would overhang the right edge. */
...
export function Badge({ provenance }: { provenance: Provenance }) {
  ...
  useEffect(() => {
    ...
    // When the badge scrolls out of sight the panel has to close, or it
    // outlives the thing it points at and floats over unrelated readouts. An
    // observer is what catches that: its intersection rect is already clipped
    // by every scrolling ancestor, so it sees a badge hidden inside a rail,
    // which a plain viewport comparison would call visible.
    ...
    // Capture phase, because the thing that scrolls is a rail or a view
    // wrapper and not the window: a bubbling listener never hears it, and the
    // panel would drift away from the badge it belongs to.
  ...
  return (
    <span className="badge-wrap">
      ...
      {open &&
        createPortal(
          /* Portalled into the body, not into the badge. Badges live inside
             ...
             clipping ancestor left, and every word of a provenance stays
             inside one rectangle wherever the badge that opened it sits. */
          <div
            ref={panelRef}
            ...
            // it in the layout while keeping the unplaced frame unseen.
            style={{
              left: pos?.left ?? 0,
              top: pos?.top ?? 0,
