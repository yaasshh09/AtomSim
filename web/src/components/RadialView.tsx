...
/** "0.01", "1", "10" rather than "1e-2". These are bohr, and bohr are familiar. */
function decadeLabel(t: number): string {
  return t >= 0.01 ? String(Number(t.toFixed(2))) : t.toExponential(0);
}

/** Below this, a peak's valley is too shallow to read as a shell boundary. */
const SHALLOW = 0.05;

/**
 * Enough decimals to carry the error bar to two significant figures.
 *
 * At a fixed three decimals, helium's 0.00034 electrons displaced prints as
 * "0.000 ± 0.000": a zero standing in for a measurement that is not zero, and
 * a resolved one at that, since the bar there is 0.00018 and the number is
 * about twice it. The bar sets the precision, because the bar is the thing
 * that says how much precision there is.
 */
...
/**
 * The disagreement in words, or the admission that it sits under the noise.
 *
 * A number smaller than its own error bar is not a measurement of anything,
 * and four decimals of it would read as precision nobody has. Helium is that
 * case: 0.0003 electrons displaced against a bar of the same size.
 */
...
/** One row of the shell table, where "no peak" is an answer rather than a gap. */
 ...
 * A radial curve.
 *
 * `logX` puts r on a log axis, which is neither decoration nor a liberty: the
 * axis is labelled with the values it carries, so nothing is hidden or
 * clipped. It stays because a many-electron atom is logarithmic in r and a
 * linear axis cannot show it. Argon's solve box runs to 48 bohr while its K
 * shell sits near 0.06 and its M shell near 1.4, so on a linear axis all three
 * shells pile into the first 3% of the width, leaving one spike and 45 bohr of
 ...
 * and says precisely nothing to anyone who does not already know it, so the
 * plain sentence goes above the axis and the engine's own label stays beside
...
function FieldPlot({
  ...
  /* The solver's grid runs far past the orbital: hydrogen's 3s is solved out
     ...
     be an axis with nothing on it. The window trims to where the curve still
     has amplitude, and the trim is disclosed under the plot with both the cut
     r and the r the grid actually reaches, because a window drawn under the
     full grid's label would be the dishonest version of this.
    ...
    <figure className="plot">
      ...
      {(nodes.length > 0 || didTrim) && (
        <p className="plot-foot">
          {nodes.length > 0 && (
            <>
              {nodes.length} radial node{nodes.length === 1 ? "" : "s"} marked on
              the axis, where the curve changes sign. The count is n − ℓ − 1, always.{" "}
            </>
          )}
          {didTrim && (
            <>
              {/* "The data sent here", not "the solve": the server windows its
                  own output to the orbital, so the last grid point here is the
...
export function RadialView() {
  ...
  useEffect(() => {
    ...
    // another is the stale render the store's INVALIDATED block exists to
  ...
  const intro = (
    <ViewIntro lead={VIEW_LEADS.radial}>
      <Disclosure summary="Why the two plots disagree at the nucleus">
        <p className="caption">
          R(r) is the wavefunction's radial part. For an s orbital it is
          largest right at r = 0, which is why the top plot starts high.
        </p>
        <p className="caption">
          ...
          shell, 4πr². At r = 0 the shell has no area, so however large R gets
        ...
        <p className="caption">
          P(r) is the one to integrate, so the second plot is the one whose area
          means something. ⟨r⟩, marked on it, is the average, and it sits to the
          right of the peak because the curve has a long tail.
        </p>
      </Disclosure>
      <Disclosure summary="How to read the wiggles">
        <p className="caption">
          ...
          3p has one and 3d has none. Raise ℓ at fixed n and the wiggles trade
          for angular structure, which is what the 3-D cloud view draws.
        </p>
      </Disclosure>
    </ViewIntro>
  );
  ...
  return (
    <div className="view-wrap">
      ...
      {radial.total_density && (
        <>
          {/* The primary curve stays on whichever model the radio selected, so
          ...
          <Disclosure summary="How much of a shell is really in one peak">
            <p className="hint-block">
              ...
              minima and the K shell comes out near 2.2, not 2, under either
              model.
            </p>
          </Disclosure>
          <Disclosure summary="What the log axis costs" tone="caveat">
            <p className="hint-block">
              r goes on a log axis because the shells are decades apart in size,
              ...
              squeezes the outer one. Read the peaks, and trust a number only
              where one is written down. The plots above are one orbital out of
          ...
          {model === "gsz" && (
            <Disclosure summary="What a fitted model gets right, and where it gives out">
              <p className="hint-block">
                ...
                and every shell solved here sees the same one. That costs less
                than it sounds. Turn the comparison on: for every atom the two
                models both cover, they place under 1.5% of the electrons
                differently, because the fit was made against Hartree-Fock in
                the first place. Where the fitted model gives out is the energy,
                not the shape.
              </p>
            </Disclosure>
          )}
