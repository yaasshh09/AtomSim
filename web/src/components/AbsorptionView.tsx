 ...
 *
 * Log in wavelength, because the lines that have to show at once span 97 nm to
 * 1876 nm. Linear in transmission, because that axis is a fraction of the
...
/**
 * The grey level for a transmission, as a CSS colour.
 *
 ...
 *
 * This panel gets drawn over two very different windows. Unzoomed it spans the
 ...
 *
 * The threshold sits at a span of 5% of the centre, which is about where four
...
export interface BandColumn {
  ...
  x: number;
  /** The deepest transmission of any sample in the column. This is what gets drawn. */
  deepest: number;
  /**
   * The flux-weighted mean over the column: what a detector pixel spanning
   * these wavelengths would actually record. Reported as a number, never
   * drawn.
   */
  mean: number;
}
 ...
 *
 * The band used to be one rectangle per grid point. The engine's grid is
 ...
 * boundary lands at about 75% of full white instead of 100%. The result was a
 * grey barcode across the whole band, including past 400 nm where the
 * transmission never drops below 0.98. That is drawing lines that are not in
 * the data, which is the one thing this app will not do.
 *
 * Two readings come out of each column, and they are wildly different, so the
 * caption has to say which one is drawn. `mean` is the honest photograph: a
 * detector pixel spanning these wavelengths integrates the flux across them,
 * so a line far narrower than a column dilutes into it. Over the whole line
 * list that is true and also useless, since every column comes back above 0.98
 * and the strip renders blank white. `deepest` is the deepest sample in the
 * column, which keeps the lines findable at the cost of overstating how dark a
 * pixel would look. So `deepest` gets drawn and labelled as such, with the
 * worst `mean` printed alongside so the photographic answer is still stated.
 */
export function bandColumns(
  ...
  // The last value carries into a column the grid does not reach, rather than
 ...
 * completely. Announcing a gap in both cases would describe a picture that is
 * not on screen half the time, so check first.
 */
...
/**
 * How much of the census this spectrum is losing, in words.
 *
 * The saturation number is the payload of the whole view, and a bare ratio
...
export function AbsorptionView({
  ...
  // Offsets go through the same log scale as everything else. Over a window
  // this narrow, log is linear to well under a pixel, so the ticks come out
  // evenly spaced. Over a wide one this branch is not taken.
  ...
  // The two readings of the band: what gets drawn, and what a real pixel of
  // the same width would record. They agree on a zoom and diverge on the full
  // range.
      ...
      <svg viewBox={`0 0 ${W} ${H + BAND_H}`} style={{ minWidth: W }} role="img" className="levels-svg">
        ...
        {mode === "log" ? (
          ...
          /* Same treatment as the zoomed line profile beside it: name the
             centre once, then label the ticks as offsets from it. */
          <text
