...
/**
 * The control vocabulary the instrument views share.
 *
 * Before this, each view invented its own: LevelsView packed four controls
 * into the same flex row as its title, SpectrumView stacked nine bare
 * checkboxes that revealed one another with nothing to say which layer you
 * were in. The physics under them was fine and the arrangement was the reason
 * nobody could find it.
 *
 * Three rules hold across everything here. A control says what it does in
 * words, not in a symbol. A number says what it means beside it, so "20 T" is
 * never on screen without something that tells you 20 T is a lot. A control
 * that cannot act right now is disabled and says why, rather than vanishing:
 * a control that disappears is one the reader will hunt for.
 */
...
export function ControlGroup({
  ...
  hint?: string;
  children: ReactNode;
  /** `active` tints the rail when the group is doing something. */
  tone?: "plain" | "active";
}) {
  return (
 ...
 * comparable to. They are separate parameters because they are separate
 * claims: the readout is the app's, the anchor is a fact about the world, and
 * only the readout may ever be derived from a solve.
 */
export function Slider({
  ...
  disabled?: boolean;
  /** True when the slider is at its "nothing applied" position. */
  atRest?: boolean;
  tourId?: string;
}) {
  return (
...
/**
 * A checkbox with a plain-English label and a line saying what it will do.
 *
 * `why` is shown whenever it is given, in both states. A hint that only
 * appears once a box is ticked cannot help the reader decide whether to tick
 * it, which is the moment the hint is for.
 */
...
export interface ChoiceOption<T extends string> {
  ...
  hint?: string;
  disabled?: boolean;
  /** Why this option cannot be picked, shown when it is disabled. */
  disabledReason?: string;
}

/**
 * A segmented picker for a set of options that were previously mutually
 ...
 * and the option that cannot run says so on its face.
 */
