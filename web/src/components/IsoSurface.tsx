 ...
 * it: the positions are the engine's interpolated crossings and the indices
 * are the engine's triangles, both untouched. What gets added on top is
 * disclosed in ISOSURFACE_LIBERTY, and the important part is the smoothing: a
...
export function IsoSurface({ vertices, triangles, colors }: Props) {
  const geometry = useMemo(() => {
    ...
    // VISUAL LIBERTY: normals are averaged across the facets sharing a vertex.
    g.computeVertexNormals();
    return g;
  }, [vertices, triangles, colors]);
  return (
    // The same rotation as the point cloud: physics z drawn screen-vertical. A
    // surface that disagreed with the cloud it sits inside would be worse than
