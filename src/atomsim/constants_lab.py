"""The What-If Lab: the observable consequences of altering the raw constants.

The five fundamental constants (hbar, e, m_e, eps0, c) arrive as multipliers
on their real CODATA values. From an altered FundamentalConstants come the
three quantities that are actually observable for a one-electron atom
against fixed SI rulers: the fine-structure constant alpha, the Bohr radius
(size), and the Hartree energy (binding). Each comes back as a Quantity whose
fidelity is COUNTERFACTUAL when any multiplier departs from 1 and EXACT
otherwise. The per-observable `changed` flag IS the degeneracy lesson: many
...
class DerivedObservable:
    ...
    changed: bool  # true when the ratio is not close to 1
...
def analyze_constants(
    ...
    """alpha, the Bohr radius (pm) and the Hartree energy (eV) from the multipliers."""
    ...
    # 1 eV = (the real elementary charge) joules, a fixed SI ruler that never changes.
    joule_per_ev = real.e
