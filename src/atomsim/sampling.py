"""How |psi_nlm|^2 is sampled by Monte-Carlo. Sampling IS physics, so it
carries provenance.

The inverse-CDF draw is factorized: r from P(r) = r^2 R_nl^2 and cos(theta)
from the normalized |Theta_lm|^2 in both bases. phi is drawn uniformly in the
complex basis, because |Y_lm|^2 does not depend on it, and from the analytic
...
class SampleCloud:
    """The positions sampled from |psi_nlm|^2, in bohr, with their provenance."""
...
def _radial_inverse_cdf_tabulated(r_grid: np.ndarray, R_values: np.ndarray):
    """The r grid and the CDF of P(r) = r^2 R^2, from a tabulated R_nl."""
...
def _radial_inverse_cdf(n: int, l: int, Z: int, mu_ratio: float):
    """The r grid and the CDF of P(r) = r^2 R_nl^2, from the analytic R_nl."""
...
def _costheta_inverse_cdf(l: int, m: int):
    """The x = cos(theta) grid and CDF of |Theta_lm|^2. The normalization cancels."""
...
def _phi_inverse_cdf(m: int):
    """The phi grid and CDF of the real-basis phi marginal (cos^2/sin^2 type)."""
...
def _draw_positions(count, r_grid, r_cdf, x_grid, x_cdf, phi_sampler, seed, n_chunks, progress):
    """Draw `count` Cartesian positions (bohr) from the factorized CDFs."""
...
def sample_density(
    ...
    """Draw `count` positions from |psi_nlm|^2 in the named angular basis."""
...
def sample_screened_density(
    ...
    *,
    ...
    The radial source is the numerical screened R_nl, and the angular part is
    the same central-field Y_lm hydrogen uses. The tier is APPROXIMATION,
...
def sample_hf_density(
    ...
    *,
    ...
    This keeps the same shape as sample_screened_density, and for the same
    reason: the radial source is a tabulated numerical R_nl and the angular part
    is the central-field Y_lm, so only the first line differs between the two
    models. Keeping them identical below that line is what puts the two models
    in the same camera.

    The fidelity is inherited from the solve rather than asserted here, so the
    two counterfactual switches carry through to the badge on the cloud.
    """
    validate_quantum_numbers(n, l)
    if abs(m) > l:
        raise ValueError(f"|m| must be <= l, got m={m}, l={l}")
