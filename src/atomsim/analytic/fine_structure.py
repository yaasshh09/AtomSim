"""Perturbative fine structure: spin-orbit + relativistic KE + Darwin, to
order alpha^2.

This is the standard combined Pauli result, a function of n and j only:
    Delta E = -(mu' Z^4 alpha^2 / (2 n^4)) (n/(j+1/2) - 3/4)   [hartree]
It is APPROXIMATION tier by construction, and the three known neglected scales
(alpha^4 terms, nuclear recoil O(m/M), electron g-2) go into the error
estimate.
"""
...
def fine_structure_shift(
    ...
    """The fine-structure energy shift Delta E(n, l, j), in hartree.

    The tier is APPROXIMATION at the real alpha and COUNTERFACTUAL once alpha
    has been altered, which is what the What-If constants lab does. The `alpha`
    argument is the seam a future FundamentalConstants.alpha will supply, so no
    signature change will be needed then.
    """
    ...
    return Quantity(
        ...
        provenance=Provenance(
            ...
            refinement=(
                "solving Dirac hydrogen exactly instead "
...
def level_energy(
    ...
    """The Bohr energy plus the fine-structure shift, in hartree.

    The fidelity follows the shift: APPROXIMATION at real alpha,
