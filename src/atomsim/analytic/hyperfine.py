...
# The s-state electron total angular momentum (l = 0, s = 1/2).
_J_S = 0.5
...
def hyperfine_constant(
    ...
    # The neglected physics: the relativistic (Z alpha)^2, plus a bound-QED and
    # nuclear-structure floor (~1e-4). This upper-bounds the true residual
    # (~6e-5 for H, ~1e-4 for He-3), so it never claims more precision than it has.
...
# --- the known nuclei, keyed by system preset --------------------------------
