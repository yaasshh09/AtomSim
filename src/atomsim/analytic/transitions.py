...
def _dipole_quadrature(n: int, l: int, n2: int, l2: int, kappa: float, nodes: int) -> float:
    ...
    r = x / a
    # The rule carries an exp(-x) weight, and the wavefunctions already supply
...
def dipole_radial_integral(
    ...
    kappa = Z * mu_ratio
    # It is symmetric in the two states, so canonicalize and let both
...
def oscillator_strength_fine(
    ...
    if abs(l_up - l_low) != 1:
        return _forbidden("Delta l != +/-1", label, "dimensionless")
    # The same 6j with its columns swapped: here the weight is the upper
    # level's degeneracy.
