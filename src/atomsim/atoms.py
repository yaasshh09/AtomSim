"""Elements, subshells, and Aufbau configurations for screened atoms
(Phase 6).

This is pure data and combinatorics: no physics engine runs here. A
Configuration is an ordered tuple of ((n, l), occupancy) in Madelung filling
order. The screened potential depends only on (Z, N), and the configuration
decides which of the computed orbitals are occupied, and so what the energy
sums over. See docs/specs/2026-07-18-phase6-screened-atoms-design.md.
"""
...
def _madelung_order() -> list[Subshell]:
    """The (n, l) shells, sorted by (n + l, n): the Madelung/Aufbau rule."""
...
def validate_config(config: Configuration, pauli: bool = True) -> None:
    ...
    all, and a subshell with n <= l offers no orbital to put an electron in
...
def _microstate_census(l: int, q: int) -> Counter[tuple[int, int]]:
    """The (M_L, 2*M_S) census of the determinants of one l^q subshell.
...
def subshell_terms(l: int, q: int) -> tuple[tuple[int, int], ...]:
    ...
    This is the textbook peel: take the largest M_S still in the census, then
    the largest M_L that survives at it, call that a term (L, S), strike its
    whole (2L+1)(2S+1) rectangle of microstates, and repeat until the census is
    empty. p^2 gives 3P, 1D, 1S; p^3 gives 4S, 2D, 2P; p^1 and p^5 give a bare
    2P.

    S comes back doubled, for the same reason the census carries it that way: a
    spin of 3/2 is exact as the integer 3 and inexact as the float 1.5.

    This is combinatorics rather than a computed result, so the return is plain
    ints. The terms follow from the Pauli principle alone, and no model sits in
    here to be honest or dishonest about.
    """
...
def subshell_term_count(l: int, q: int) -> int:
    """How many Russell-Saunders terms l^q spans. 1 leaves nothing to average."""
...
def is_single_term(config: Configuration) -> bool:
    ...
    This decides whether the "average of configuration" is a real limitation or
    an empty one. A configuration with one term leaves nothing to average: its
    configuration-average energy IS that term's energy. Closed subshells span
    only 1S; a lone open subshell is the interesting case, so it gets counted;
...
class Element:
    ...
    #: shows up in a real spectrum as an isotope shift this model leaves out.
    mass_u: float


#: The standard atomic weights, from the IUPAC/CIAAW 2021 table, abridged to
#: the digits that matter here: a Doppler width goes as 1/sqrt(m), so the fourth
#: digit moves it by parts in ten thousand, far below the model's own error.
...
# The elements with no published neutral GSZ screening parameters. Szydlik &
...
# Ar^+). Rather than invent parameters, these atoms stay out of the preset
# library: the prime directive forbids quietly shipping unsourced physics.
#
...
# Every nameable many-electron atom, neutral He..Ar. Membership means "this key
# identifies an atom", and nothing more.
#
# It used to mean "this key selects the screened model" as well, and the two
# were the same list because GSZ was the only model that could draw anything.
# Once Hartree-Fock reached every view that stopped being true, and conflating
# the two cost something visible: sulfur and chlorine were missing from the
...
# Which atom is solvable and which atom one model has parameters for are two
# different questions, so there are two lists.
ATOM_KEYS: tuple[str, ...] = tuple(
    e.symbol.lower() for e in ELEMENTS if e.z >= 2
)

# The atoms the GSZ screened model can speak for: neutral He..P and Ar. A caller
