"""Isosurfaces of |psi|^2, at the level that encloses a stated fraction of the
electron.
...
rather than picked, and the answer carries the part textbooks leave out: the
electron is outside this surface one time in ten.

Two things can turn that number into a lie, and both are measured rather than
...
it. So the box grows until it holds essentially everything, and what still
escapes is reported. The second is the grid: the level that encloses 90% of a
Riemann sum is not exactly the level that encloses 90% of the integral, so the
solve is repeated at half the resolution and the difference becomes the error
bar.

The geometry lives in `numerics/marching_tets.py` and knows nothing about atoms.
"""
...
#: The grid sizes a surface can be built on. Odd counts put a grid point exactly
#: on the nucleus, where 1s has its cusp, which is where the Riemann sum is least
#: forgiving; even counts straddle it. Both are offered and neither hidden, since
#: the resolution appears in the provenance.
GRID_SIZES = (48, 64, 96, 128)

#: How much of the electron the box must hold before a level is solved on it.
#: The remainder is disclosed rather than quietly rolled into the answer.
_BOX_CAPTURE = 0.999

#: The radial samples used to measure the tail. They stay uniform, so the
#: trapezoid error is h^2 on a smooth r^2 |R(r)|^2, and 1024 intervals over tens
#: of bohr leave it far below the 1e-3 the box is sized to.
_TAIL_SAMPLES = 1024
...
class Isosurface:
    """A drawn closed surface of |psi|^2, and the measurements that make it a
    claim.

    A container rather than a `Field`: a mesh is not samples on a 1-D grid, and
...
def default_half_width(n: int, Z: int = 1, mu_ratio: float = 1.0) -> float:
    """The starting box for a hydrogen-like state, in bohr.

    Deliberately generous rather than tuned: it is only a starting point for the
    growth loop below, and that loop stops on a measurement. For 1s it gives 8
    bohr where 99.9% of the electron is inside 5.61, and guessing high costs a
    longer radial profile, not a wrong answer.
    """
    return (2.0 * n * n + 6.0 * n) / (Z * mu_ratio)


def _density_on_grid(evaluator, half_width: float, resolution: int, progress=None):
    """|psi|^2 on the cubic grid, plus the psi assumptions the evaluator declares."""
...
def radial_mass(evaluator, l: int, m: int, r_max: float, samples: int = _TAIL_SAMPLES):
    """The probability enclosed within each radius out to `r_max`, by spherical
    average.
    ...
    faster than the tail shrinks: for a hydrogen 1s the sum fell from 0.997 at 8
    ...
    # The weights average over the sphere: sum(w_u) = 2 for the u integral and
...
def _fit_box(evaluator, l: int, m: int, start: float, growths: int = 8):
    """The smallest radius holding `_BOX_CAPTURE` of the electron, and the profile.

    The test is on the inscribed sphere rather than the cube, so the box handed
    back holds at least the stated fraction and the escaped mass reported from it
...
def solve_level(grid: np.ndarray, cell_volume: float, target: float) -> float:
    ...
    The cell densities are sorted descending, `rho dV` accumulated, and the
    crossing of the running total interpolated. That is exact on the grid, one
...
def fraction_above(grid: np.ndarray, cell_volume: float, level: float) -> float:
    """The probability the region at or above `level` holds, on this grid."""
...
def _phase_at(evaluator, vertices: np.ndarray) -> np.ndarray:
    ...
    The surface runs through |psi|^2, which is blind to sign, so this is what
    gives a p orbital its two-coloured lobes. In the real basis psi is real and
    the answer is 0 or pi; in the complex basis it is the true phase, and it
    feeds the same hue map the point cloud uses.
    """
...
def _build(
    ...
    *,
    ...
    # The tail is measured out to at least the box actually used, so a
    # hand-set box gets the same measured escape a fitted one does.
    ...
    # The grid is halved twice over, because the two things this surface claims
    # do not converge at the same rate and one of them is nearly blind.
    #
    ...
    # near zero. That is a true statement about the fraction and it would be a
    ...
    coarse = grid[::2, ::2, ::2]
    try:
        # Every second point, so the cells are twice as wide and eight times as big.
...
def isosurface(
    ...
    """The enclosed-probability isosurface of a hydrogen-like |psi_nlm|^2.

    The tier is `NUMERICAL` even though psi is closed form: the grid and the
    linear interpolation along cell edges are the approximation here, and no
    exactness upstream makes a triangulated contour exact.
    """
...
def screened_isosurface(
    ...
    The tier is `APPROXIMATION`, the weaker of the two: the screening model is a
    bigger departure from the real atom than the grid is from the model.
    """
    validate_quantum_numbers(n, l)
    validate_angular(l, m)
...
def hf_isosurface(
    ...
    *,
    ...
    The tier is `APPROXIMATION`, the weaker of the two, for the reason
    screened_isosurface gives: the model's distance from the real atom is larger
    than the grid's distance from the model. With either counterfactual switch
    thrown it is COUNTERFACTUAL instead, read off the solve rather than decided
    here.
    """
    validate_quantum_numbers(n, l)
    validate_angular(l, m)
