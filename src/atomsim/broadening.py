...
#: Cap on profile evaluations (lines times grid points). A spectrum appears
#: while someone waits, so a coarser grid beats a hung request; when that
#: happens, the provenance says so.
...
#: How far, in FWHM, a line is summed before its wings are dropped. Generous on
#: purpose: at this distance a Lorentzian tail is about 1e-6 of the line.
_CUT_FWHM: float = 1e5

#: Cap on the grid points evaluated for one line, so a very broad line in a
#: long list cannot bring back the quadratic cost the cut exists to avoid.
...
_NOT_MODELLED = (
    ...
    "at attainable densities, so its size is reported separately rather than "
    "folded in",
...
class LineProfile:
    ...
    #: Total Voigt FWHM, nm.
    fwhm_nm: float
    #: The area placed under this line, in the weight's unit.
    weight: float
...
class SyntheticSpectrum:
    ...
    #: order. They are kept because a profile does not identify its line:
    #: degenerate transitions share a wavelength exactly (all of 3d->2p, 3p->2s,
    #: 3s->2p sit at 656.47 nm), so the pairing is handed over rather than left
    #: for anything to reconstruct from what it can see.
    ...
    #: 1 is the grid's own quadrature error, measured rather than assumed. Below
    #: 1 also picks up flux that fell outside the window.
    flux_closure: float = 1.0
    #: The size of the collisional broadening left out, for the strongest line
    #: in the list. None when it does not apply (no plasma conditions were
    #: given, or a non-hydrogenic atom has no linear Stark effect).
...
def stark_span_estimate(
    ...
    return Quantity(
        ...
        provenance=Provenance(
            ...
            method=(
                "the peak-to-peak span of the linear Stark components at "
                "the Holtsmark normal field "
                "F_0 = 2.603 e n_e^(2/3) / (4 pi eps_0); "
                f"F_0 = {f_0:.3g} V/m here"
            ),
            assumptions=(
                "quasi-static approximation: the perturbing ions are frozen "
                "while the atom radiates",
...
def synthesize(
    ...
    # The lower level's decay rate comes out of the same table: it is an upper
    ...
    for ln in lines:
        ...
        # state, is metastable. It gets exactly zero natural width.
    ...
    if all(p.fwhm_nm <= 0.0 for p in profiles):
        raise ValueError(
            "every line in this list has zero width: no decay rate "
            "(turn on intensities), no temperature (turn on LTE weighting), "
            "and no instrument (set a resolving power). A profile would have to "
            "be invented, so none is drawn"
        )
    ...
    if window_nm is not None:
        ...
        # The widest line in the list sets the pad, and with fine structure
        # that is a within-n component out at metre wavelengths whose thermal
        # width alone is kilometres. Subtracting that from the bluest line walks
        # the window past zero, and a wavelength grid does not have a negative
        # end. So it is floored at half the bluest line, which keeps a real
        # margin without inventing negative light.
        lo, hi = max(lo - pad, 0.5 * lo), hi + pad
    # Zero-width lines (a 2s lower level with no E1 channel, at T = 0) would
    # collapse their own cluster onto one point; they get the list's median
    ...
    # The grid is capped for the payload's sake, not the arithmetic's: sampling
    ...
    coarsened = False
    # The cheapest thing goes first: the baseline between lines, then the far
    # wings, and only last the core, which is the only part that decides what
    # the peak looks like.
    ...
    # evaluations. Cutting the wings is the compromise, and the cut is paid for
    # honestly: the neglected flux is computed analytically from the tail
    # integrals and reported, rather than assumed small.
    #
    ...
    # FWHM that is around 1e-6 of a line, but the number is measured for the cut
    ...
    assumptions = [
        "a Voigt profile (Gaussian widths in quadrature, Lorentzian widths "
        "added) evaluated with the Faddeeva function; exact given the widths",
        "the grid is adaptive (a cluster per line over a coarse background); "
        "each line's own centre is a grid point, so no peak is undersampled",
        f"each line is summed out to {_CUT_FWHM:.0e} of its own FWHM, with its "
    ...
    if metastable:
        assumptions.append(
            ...
            "E1 decay channel at all, so they get exactly zero natural width. "
            ...
            "right, but the level is not the infinitely sharp thing implied"
        )
    if coarsened:
        assumptions.append(
            f"the grid was coarsened to stay inside the evaluation budget "
    ...
    # What is missing, sized. It is reported for the strongest line, since that
    # is the one being looked at, and flagged only when it is big enough to
    # change the shape being read.
    ...
    if thermal is not None and hydrogenic:
        ...
        if stark_span.value >= 0.3 * strongest.fwhm_nm:
            stark_note = (
                ...
                f"{strongest.fwhm_nm:.3g} nm modelled here. The curve is "
    ...
    return SyntheticSpectrum(
        spectrum=Field(
            ...
            provenance=Provenance(
                # The Voigt evaluation is exact; every uncertainty here rides
                # in on the widths (E1-only rates, a Maxwellian, LTE conditions).
                fidelity=Fidelity.APPROXIMATION,
                method=(
                    "summed area-normalized Voigt line profiles; the area under "
                    f"each line = its {weight_kind}"
                ),
                assumptions=tuple(assumptions) + _NOT_MODELLED,
                refinement=(
                    "collisional broadening, self-absorption, and a velocity "
                    "field would each add width this curve does not have"
                ),
            ),
        ),
