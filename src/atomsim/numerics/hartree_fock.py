...
`exchange=False` turns Hartree-Fock into Hartree, the counterfactual model
...
A half-applied flag is caught for free: hf_atom.py's two total-energy routes run
...
# Exchange turned off, expressed as an operator rather than as an absence.
# It is frozen and empty, so it is shared across every channel of every Hartree
# solve and applies as a zero with no special case anywhere downstream.
...
class ChannelSolution:
    ...
    # stays falsifiable. See the note in solve_channel:
    ...
    # rather than asserted away, because the grid and
    # whether exchange is on decide what is attainable.
...
def fock_operator(
    ...
    *,
    ...
    See the module docstring for what has to move with it.
    """
    return _fock_parts(
...
def _fock_parts(
    ...
    *,
    ...
    # The empty operator beats a branch inside matvec: matvec runs
    # hundreds of times per channel, and this settles the question before any
    # of them. An ExchangeOperator with no terms applies as an exact zero.
    exchange_op = (
...
def solve_channel(
    ...
    *,
    ...
    # The request is scaled by the channel's energy magnitude, for the same
    # reason the ceiling is. An absolute 1e-6 is unreachable in a channel whose
    ...
    # and the measurements say the opposite: the extra vectors land in the
    ...
    # iteration budget on vectors that then get thrown away. Argon's 3s channel
    ...
    def warm_block() -> np.ndarray:
        # The guess arrives in P, the physical radial function, because that
        # is what callers hold; the solve happens in S.
        ...
        #
        # The rule this replaced padded with local states from index len(guess)
        # upward, which assumed a partial guess covers the LOWEST states. In
        # this SCF it covers the highest: the 3s channel is solved for three
        # states and warm-started with the 3s alone. That put a near duplicate
        # of the guess in the block AND left no approximation to the 1s, so
    ...
    def attempt(x: np.ndarray, budget: int):
        ...
        # block is scratch, and letting scratch fail the gate would reject
        # solves that are converged in every eigenvalue the caller receives.
    ...
    # attempt is the committed one, so it gets double. Measured on beryllium:
    # a stagnating warm start allowed all 400 iterations costs 200 seconds,
    ...
    # channel into a hard failure, which is why they stay different.
    starts = (
    ...
    for attempt_index, (build, budget) in enumerate(starts):
        ...
        if np.isfinite(residual) and residual <= limit:
            break
        # A warm start is an optimization, not a commitment. When the
        ...
        # of the grid, the local spectrum is a known-good block to fall
        # back to. The retry costs nothing on the overwhelming majority of
    ...
    # Modified Gram-Schmidt in the mesh's inner product, then fix the
...
def _interaction_energy(
    ...
    same-shell F_k and every cross-shell G_k, and exchange=False drops
    exactly those and keeps the k = 0 Hartree repulsion. The (q_a - 1)
