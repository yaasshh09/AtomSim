...
Same as Phase 26: `Quantity`/`Field` with `Provenance` at every boundary, tiers inherited from the solve and never asserted, Hartree atomic units internally, line length 100, `ruff check .` clean, no em dashes, new physics gets a validation test, commit per logical change on `main`, no AI attribution.
